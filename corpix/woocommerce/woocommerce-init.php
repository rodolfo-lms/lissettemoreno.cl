...
if (!class_exists('Corpix_Woocoommerce') ) {
    /**
     * Corpix Woocommerce
     *
     *
     * @package corpix\woocoomerce
     * @author Pixelcurve <help.pixelcurve@gmail.com>
     * @since 1.0.0
     */
    class Corpix_Woocoommerce
    {
        private $row_class;
        ...
        public function setup()
        {
            ...
            add_theme_support(
                'woocommerce',
                apply_filters(
                    'corpix_woocommerce_args',
                    [
                        'single_image_width' => 1140,
                        'thumbnail_image_width' => 480,
                        'gallery_thumbnail_image_width' => 270,
                        'product_grid' => [
                            'default_columns' => (int) Corpix::get_option('shop_column'),
                            'default_rows' => 4,
                            'min_columns' => 1,
                            'max_columns' => 6,
                            'min_rows' => 1,
        ...
        public function init ()
        {
            ...
            // Page Template
            add_action('woocommerce_before_main_content', [$this, 'tpc_page_template_open'], 10);

            // ↓ Wrapper Sorting
            add_action('woocommerce_before_shop_loop', [$this, 'tpc_sorting_wrapper_open'], 9);
            add_action('woocommerce_before_shop_loop', [$this, 'tpc_sorting_wrapper_close'], 31);
            // ↑ wrapper sorting
            ...
            add_action('woocommerce_before_shop_loop', [$this, 'tpc_product_columns_wrapper_open'], 40);
            add_action('woocommerce_after_shop_loop', [$this, 'tpc_product_columns_wrapper_close'], 40);

            add_filter('comment_form_fields', [$this, 'tpc_comments_fiels']);
            add_filter('woocommerce_product_review_comment_form_args', [$this, 'tpc_filter_comments'], 10, 1);
            add_filter('woocommerce_product_review_list_args', [$this, 'tpc_filter_reviews'], 10, 1);
            add_filter('woocommerce_review_gravatar_size', [$this, 'tpc_review_gravatar_size'], 10, 1);

            add_filter('woocommerce_cart_item_thumbnail', [$this, 'tpc_image_thumbnails'], 10, 3);

            // Filter pagination
            add_filter('woocommerce_pagination_args', [$this, 'tpc_filter_pagination']);
        ...
        /* RT Reviews filter */
        /**/
        function tpc_filter_reviews($array)
        {
            return [ 'callback' => [ $this, 'tpc_templates_reviews' ] ];
        }

        public function tpc_templates_reviews($comment, $args, $depth)
        {
        ...
        public function tpc_review_gravatar_size()
        {
            return 120;
        }
        ...
        function review_comments_meta_info($comment)
        {
            ...
            if ('0' === $comment->comment_approved) { ?>
                <em class="woocommerce-review__awaiting-approval">
                    <?php esc_html_e('Your review is awaiting approval', 'corpix'); ?>
                </em>
            ...
            <?php } else { ?>
                ...
                if ('yes' === get_option( 'woocommerce_review_rating_verification_label' ) && $verified) {
                    echo '<em class="woocommerce-review__verified verified">(' . esc_attr__('verified owner', 'corpix') . ')</em> ';
                }
                ?>
                <div class="meta-data">
        ...
        /**/
        function tpc_filter_comments($comment_form)
        {
            $commenter = wp_get_current_commenter();

            $comment_form = [
                'title_reply' => have_comments() ? esc_html__('Add a review', 'corpix') : sprintf( esc_html__('Be the first to review &ldquo;%s&rdquo;', 'corpix'), get_the_title() ),
                'title_reply_to' => esc_html__( 'Leave a Reply to %s', 'corpix' ),
                'title_reply_before' => '<h3 id="reply-title" class="comment-reply-title">',
                'title_reply_after' => '</h3>',
                'comment_notes_after' => '',
                'fields' => [
                    'author' => '<p class="comment-form-author">' . '<label for="author"></label> ' .
                    '<input id="author" name="author" placeholder="'.esc_attr__( 'Your Name', 'corpix' ).'" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" required /></p>',
                    'email' => '<p class="comment-form-email"><label for="email"></label> ' .
                    '<input id="email" name="email" placeholder="'. esc_attr__( 'Your Email', 'corpix' ).'" type="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="30" required /></p>',
                ],
                'label_submit' => esc_html__('Submit', 'corpix'),
                'logged_in_as' => '',
                'comment_field' => '',
            ];
            ...
            if ($account_page_url = wc_get_page_permalink('myaccount')) {
                ...
                $comment_form['must_log_in'] = '<p class="must-log-in">' . sprintf( wp_kses( __( 'You must be <a href="%s">logged in</a> to post a review.', 'corpix' ), $allowed_html), esc_url( $account_page_url ) ) . '</p>';
            }

            if (get_option('woocommerce_enable_review_rating') === 'yes') {
                $comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__('Your Rating For This Product', 'corpix') . '</label><select name="rating" id="rating" required>
                <option value="">' . esc_html__('Rate&hellip;', 'corpix') . '</option>
                <option value="5">' . esc_html__('Perfect', 'corpix') . '</option>
                <option value="4">' . esc_html__('Good', 'corpix') . '</option>
                <option value="3">' . esc_html__('Average', 'corpix') . '</option>
                <option value="2">' . esc_html__('Not that bad', 'corpix') . '</option>
                <option value="1">' . esc_html__('Very poor', 'corpix') . '</option>
                </select></div>';
            }

            $comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment"></label><textarea id="comment" name="comment" cols="45" rows="8" placeholder="'.esc_attr__('Your review', 'corpix').'" required></textarea></p>';
        ...
        */
        function tpc_comments_fiels($fields)
        {
            if (is_product()) {
        ...
        {
            return (int) Corpix::get_option('shop_products_per_page');
        }
        ...
        {
            $args = [
                'posts_per_page' => (int) Corpix::get_option('shop_r_products_per_page'),
                'columns' => (int) Corpix::get_option('shop_related_columns'),
            ];
        ...
        {
            $columns = (int) Corpix::get_option('shop_column'); // 3 products per row

            return $columns;
        }

        public function tpc_product_columns_wrapper()
        {
            $columns = (int) Corpix::get_option('shop_column');
            echo '<div class="tpc-products-catalog tpc-products-wrapper columns-' . absint($columns) . '">';
        }
        ...
        public function tpc_sorting_wrapper_open()
        {
            echo '<div class="tpc-woocommerce-sorting">';
        }

        public function tpc_sorting_wrapper_close()
        {
            echo '</div>';
        }

        public function tpc_product_columns_wrapper_open()
        {
            $columns = (int) Corpix::get_option('shop_column');
            echo '<div class="tpc-products-catalog tpc-products-wrapper columns-' . absint($columns) . '">';
        }

        public function tpc_product_columns_wrapper_close()
        {
            echo '</div>';
        }
        ...
        public function get_sidebar_data()
        {
            ...
            return Corpix::get_sidebar_data('shop_' . $shop_template);
        }

        public function tpc_page_template_open()
        {
            $sb = $this->get_sidebar_data();

            echo '<div class="tpc-container single_product', esc_attr($sb['container_class'] ?? ''), '">',
                '<div class="row', esc_attr($sb['row_class'] ?? ''), '">',
                '<div id="main-content" class="tpc_col-', (int) esc_attr($sb['column'] ?? ''), '">';

            add_action('woocommerce_after_main_content', function () use ($sb) {
                echo '</div>';
                if ($sb) { Corpix::render_sidebar($sb); }
                echo '</div>';
                echo '</div>';
            }, 10);
        }

        public function tpc_filter_pagination()
        {
        ...
        public function tpc_image_thumbnails($image, $cart_item, $cart_item_key)
        {
            $class = 'attachment-woocommerce_thumbnail size-woocommerce_thumbnail tpc-woocommerce_thumbnail'; // Default cart thumbnail class.
            ...
            if(function_exists('aq_resize') && !!$image_url){
                ...
                $width = $height = apply_filters('tpc_woo_mini_thumbnail_size', '70');
        ...
        public function woo_rename_tabs( $tabs ) {
	        global $product;
		    $tabs['reviews']['title'] = esc_html__( 'Reviews', 'corpix' ).' <span class="count">('. $product->get_review_count().')</span>';
		    return $tabs;
	    }
    }
}
...
new Corpix_Woocoommerce();

// Corpix Woocoommerce Helpers

if (!function_exists('corpix_woocommerce_breadcrumb')) {
	/**
	 ...
	 * @return string
	 */
    function corpix_woocommerce_breadcrumb($args = [])
    {
        $args = wp_parse_args($args, apply_filters('woocommerce_breadcrumb_defaults', [
            ...
            'before' => '',
            'after' => '',
            'home' => esc_html_x('Home', 'breadcrumb', 'corpix'),
        ]));
        ...
        if (!empty($breadcrumb)) {
            ...
            $out .= Corpix::render_html($wrap_before);

            foreach ($breadcrumb as $key => $crumb) {

                $out .= Corpix::render_html($before);
                ...
                $out .= Corpix::render_html($after);

                if (sizeof($breadcrumb) !== $key + 1) {
                    $out .= Corpix::render_html($delimiter);
                }
            }
            $out .= Corpix::render_html($wrap_after);
        }
