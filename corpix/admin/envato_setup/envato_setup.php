...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function init_globals() {
            ...
            $this->oauth_script    = apply_filters( $this->theme_name . '_theme_setup_wizard_oauth_script', '//www.pixelcurve.com/envato/api/server-script.php' );
        ...
        public function admin_menus() {
         add_submenu_page('corpix-admin-menu', esc_html__( 'Setup Wizard','corpix' ), esc_html__( 'Setup Wizard','corpix' ), 'manage_options', $this->page_slug,  array( $this, $this->page_slug) );
     }
        ...
        public function init_wizard_steps() {
            ...
            $this->steps = array(
                'introduction' => array(
                    'name'    => esc_html__( 'Introduction','corpix' ),
        ...
        public function setup_wizard() {
            ...
            wp_localize_script( 'envato-setup', 'envato_setup_params', array(
                ...
                'verify_text'      => esc_html__( '...verifying','corpix' ),
            ) );
        ...
        public function setup_wizard_header() {
            ...
            // avoid theme check issues.
                echo '<t';
                echo 'itle>' . __( 'Theme &rsaquo; Setup Wizard','corpix' ) . '</ti' . 'tle>'; ?>
            ...
            <body class="envato-setup wp-core-ui">
                <h1 id="wc-logo">
                    ...
                    if ( $image_url ) {
                        ...
                        <img src="<?php echo esc_url( $this->plugin_url . 'images/logo.png' ); ?>" alt="<?php esc_attr_e( 'Envato install wizard', 'corpix' );?>" /><?php
                    } ?>
                </h1>
                <?php
            }
        ...
        public function setup_wizard_footer() {
            ...
            <?php if ( 'next_steps' === $this->step ) : ?>
                ...
                href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the Dashboard','corpix' ); ?></a>
            <?php endif; ?>
        </body>
        <?php
        ...
        public function envato_setup_introduction() {
            ...
            if ( false && isset( $_REQUEST['debug'] ) ) {
                ...
                ?>
                <h1><?php printf( __( 'Welcome to %s.','corpix' ), wp_get_theme() ); ?></h1>
                <p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.','corpix' ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                     class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!','corpix' ); ?></a>
                     <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                         class="button button-large"><?php esc_html_e( 'Not right now','corpix' ); ?></a>
                     </p>
                     <?php
                 } else if ( get_option( 'envato_setup_complete', false ) ) {
                    ?>
                    <h1><?php printf( __( 'Welcome to %s.','corpix' ), wp_get_theme() ); ?></h1>
                    <p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ','corpix' ); ?></p>
                    <ul>
                        <li>
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                             class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again','corpix' ); ?></a>
                         </li>
                     </ul>
                     <p class="envato-setup-actions step">
                        <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                         class="button button-large"><?php esc_html_e( 'Cancel','corpix' ); ?></a>
                     </p>
                     <?php
                 } else { ?>
        ...
        public function envato_setup_introduction_save() {
            ...
            if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
                ...
                ?>
                <p>
                    <strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.','corpix' ); ?></strong>
                </p>
                <?php
                return true;
            }
        ...
        public function envato_setup_default_plugins() {
            ...
            ?>
            <h1><?php esc_html_e( 'Default Plugins','corpix' ); ?></h1>
            <form method="post">
                ...
                if ( count( $plugins['all'] ) ) {
                    ?>
                    <p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:','corpix' ); ?></p>
                ...
                echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.','corpix' ) . '</strong></p>';
            } ?>

            <p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.','corpix' ); ?></p>

            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                 class="button-primary button button-large button-next"
                 data-callback="install_plugins"><?php esc_html_e( 'Continue','corpix' ); ?></a>
                 <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                     class="button button-large button-next"><?php esc_html_e( 'Skip this step','corpix' ); ?></a>
         ...
         public function ajax_plugins() {
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => __( 'No Slug Found','corpix' ) ) );
            }
            $json = array();
            ...
            foreach ( $plugins['activate'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Activating Plugin','corpix' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['update'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Updating Plugin','corpix' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['install'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Installing Plugin','corpix' ),
                    );
                    break;
                }
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','corpix' ) ) );
            }
            exit;
        ...
        private function _content_default_get() {
            ...
            foreach ( $available_content as $post_type => $post_data ) {
                if ( count( $post_data ) ) {
                    ...
                    $content[ $post_type ] = array(
                        ...
                        'description'      => sprintf( __( 'This will create default %s as seen in the demo.','corpix' ), $post_type_title ),
                        'pending'          => esc_html__( 'Pending.','corpix' ),
                        'installing'       => esc_html__( 'Installing.','corpix' ),
                        'success'          => esc_html__( 'Success.' ,'corpix'),
            ...
            $content['widgets'] = array(
                'title'            => esc_html__( 'Widgets','corpix' ),
                'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.' ,'corpix'),
                'pending'          => esc_html__( 'Pending.' ,'corpix'),
                'installing'       => esc_html__( 'Installing Default Widgets.','corpix' ),
                'success'          => esc_html__( 'Success.','corpix' ),
            ...
            $content['settings'] = array(
                'title'            => esc_html__( 'Settings' ,'corpix'),
                'description'      => esc_html__( 'Configure default settings.' ,'corpix'),
                'pending'          => esc_html__( 'Pending.','corpix' ),
                'installing'       => esc_html__( 'Installing Default Settings.' ,'corpix'),
                'success'          => esc_html__( 'Success.','corpix' ),
        ...
        public function envato_setup_default_content() {
            ?>
            <h1><?php esc_html_e( 'Default Content','corpix' ); ?></h1>
            <form method="post">
                <?php if ( $this->is_possible_upgrade() ) { ?>
                    <p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','corpix' ); ?></p>
                <?php } else { ?>
                    <p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ','corpix' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
                <?php } ?>
                <table class="envato-setup-pages" cellspacing="0">
                    <thead>
                        <tr>
                            <td class="check"></td>
                            <th class="item"><?php esc_html_e( 'Item','corpix' ); ?></th>
                            <th class="description"><?php esc_html_e( 'Description','corpix' ); ?></th>
                            <th class="status"><?php esc_html_e( 'Status','corpix' ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                     class="button-primary button button-large button-next"
                     data-callback="install_content"><?php esc_html_e( 'Continue','corpix' ); ?></a>
                     <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                         class="button button-large button-next"><?php esc_html_e( 'Skip this step','corpix' ); ?></a>
                         <?php wp_nonce_field( 'envato-setup' ); ?>
                     </p>
             ...
             public function ajax_content() {
                ...
                if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
                    wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found' ,'corpix') ) );
                }
                ...
                wp_send_json( array(
                    'error'   => 1,
                    'message' => esc_html__( 'Error','corpix' ),
                    'logs'    => $this->logs,
                    'errors'  => $this->errors,
                ) );
            }
        ...
        private function _fetch_remote_file( $url, $post ) {
            ...
            if ( ! $upload || $upload['error'] ) {
                ...
                if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond','corpix' ) );
                }
                ...
                if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size' ,'corpix') );
                }

                if ( 0 == $filesize ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded','corpix' ) );
                }

                if ( ! empty( $max_size ) && $filesize > $max_size ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s','corpix' ), size_format( $max_size ) ) );
                }
            }
        ...
        public function envato_setup_updates() {
            ?>
            <h1><?php esc_html_e( 'Theme Updates','corpix' ); ?></h1>
            <?php if ( function_exists( 'envato_market' ) ) { ?>
                <form method="post">
                    ...
                    if ( count( $my_items ) ) {
                        ?>
                        <p><?php esc_html_e( 'Thanks! Theme updates have been enabled for the following items:','corpix' ); ?></p>
                        <ul>
                            <?php foreach ( $my_items as $item ) { ?>
                                <li><?php echo esc_html( $item['name'] ); ?></li>
                            <?php } ?>
                        </ul>
                        <p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.','corpix' ); ?></p>
                        <p><?php esc_html_e( "Change settings from the 'Envato Market' menu in the WordPress Dashboard.",'corpix' ); ?></p>

                        <p class="envato-setup-actions step">
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                             class="button button-large button-next button-primary"><?php esc_html_e( 'Continue','corpix' ); ?></a>
                         </p>
                         <?php
                     } else {
                        ?>
                        <p><?php esc_html_e( 'ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.','corpix' ); ?></p>
                        <p><?php esc_html_e( "When an update becomes available it will show in the Dashboard with an option to install.",'corpix' ); ?></p>
                        <p>
                            <em><?php esc_html_e( 'If you have any questions please ','corpix' ); ?><a
                            href="<?php echo esc_url('https://pixelcurve.com/support/'); ?>" target="_blank"><?php esc_html_e( 'Contact us.','corpix' ); ?></a></em>
                        </p>
                        <p class="envato-setup-actions step">

                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                             class="button button-primary button-large button-next"><?php esc_html_e( 'Continue','corpix' ); ?></a>
                             
<!--                             <input type="submit" class="button-primary button button-large button-next"
                            value="<?php ///esc_attr_e( 'Login with Envato','corpix' ); ?>" name="save_step"/> -->
             ...
             <?php } else { ?>
                <?php esc_html_e( 'Please ensure the Envato Market plugin has been installed correctly.','corpix' ); ?> <a
                href="<?php echo esc_url( $this->get_step_link( 'default_plugins' ) ); ?>"><?php esc_html_e( 'Return to Required
                Plugins installer.','corpix' ); ?></a>
            <?php } ?>
            <?php
        }
        ...
        public function envato_setup_customize() {
            ...
            <h1><?php esc_html_e( 'Theme Customization','corpix' ); ?></h1>
            <p><?php esc_html_e( 'Most changes to the website can be made through the Corpix > Theme Options menu from the WordPress
            dashboard. These include Whole site global settings, typography, colors, styles, etc.','corpix' ); ?> 
        </p>
        
    <p><?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; drop" into sidebars. More details in documentation.','corpix' ); ?></p>
    <p>
        <em><?php esc_html_e( 'Advanced Users: If you are going to make changes to the theme source code please use a','corpix' ); ?> <a
            href="//developer.wordpress.org/themes/advanced-topics/child-themes/" target="_blank"><?php esc_html_e( 'Child Theme','corpix' ); ?></a> <?php esc_html_e( 'rather than
                modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without
                overwriting your source code changes.','corpix' ); ?> <br/> <?php esc_html_e( 'See','corpix' ); ?> <code><?php esc_html_e( 'corpix-child.zip','corpix' ); ?></code> <?php esc_html_e( 'in the main theme folder.','corpix' ); ?></em>
            </p>
         ...
         public function envato_setup_help_support() {
            ?>
            <h1><?php esc_html_e( 'Help and Support','corpix' ); ?></h1>
            <p><?php esc_html_e( 'This theme comes with 6 months item support from purchase date (with the option to extend this period).
                ...
                use this theme on another website.','corpix' ); ?></p>

                <p class="greendata"><?php esc_html_e( 'Item Support can be accessed from','corpix' ); ?> <a href="<?php echo esc_url('https://pixelcurve.com/support'); ?>" target="_blank"><?php esc_html_e( 'Pixelcurve Help','corpix' ); ?></a>
                 <?php esc_html_e( 'and includes:','corpix' ); ?></p>
                 <ul>
                    <li><?php esc_html_e( 'Availability of the author to answer questions','corpix' ); ?></li>
                    <li><?php esc_html_e( 'Answering technical questions about item features','corpix' ); ?></li>
                    <li><?php esc_html_e( 'Assistance with reported bugs and issues','corpix' ); ?></li>
                    <li><?php esc_html_e( 'Help with bundled 3rd party plugins','corpix' ); ?></li>
                </ul>

                <p class="reddata"><?php esc_html_e( 'Item Support DOES NOT Include:', 'corpix' ); ?></p>
                <ul>
                    <li><?php esc_html_e( 'Customization services', 'corpix' ); ?>
                </li>
            </li>
        </ul>
        <p><?php esc_html_e( 'More details about item support can be found in the ThemeForest', 'corpix' ); ?> <a
            href="https://themeforest.net/page/item_support_policy" target="_blank"><?php esc_html_e( 'Item Support Polity.', 'corpix' ); ?></a> </p>
            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                 class="button button-primary button-large button-next"><?php esc_html_e( 'Continue','corpix' ); ?></a>
                 <?php wp_nonce_field( 'envato-setup' ); ?>
        ...
        public function envato_setup_ready() {
            ...
            <h1><?php esc_html_e( 'Your website is almost ready!','corpix' ); ?></h1>

            <p class="greendata"><?php esc_html_e( 'Congratulations! The theme and the required plugins have been activated. Click the "Import Dummy Data" button for demo import.','corpix' ); ?></p>


                <div class="envato-setup-next-steps">
                    <div class="envato-setup-next-steps-first">
                        <h2><?php esc_html_e( 'Next import dummy data','corpix' ); ?></h2>
                        <ul>
                            ...
                            <li class="setup-product"><a class="button button-primary button-large"
                               href="<?php echo esc_url($url); ?>"><?php esc_html_e( 'Import Dummy Data', 'corpix' ); ?></a>
                           </li>
                           <li class="setup-product"><a class="button button-next button-large"
                               href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'View your new website!','corpix' ); ?></a>
                           </li>
                           <li class="setup-product"><a class="button button-next button-large"
                               href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the Dashboard','corpix' ); ?></a>
                           </li>
                   ...
                   <div class="envato-setup-next-steps-last">
                    <h2><?php esc_html_e( 'More resources','corpix' ); ?></h2>
                    <ul>
                        <li class="documentation"><a href="<?php echo esc_url( 'https://pixelcurve.com/docs/corpix/' ); ?>"
                           target="_blank"><?php esc_html_e( 'Read the Theme Documentation','corpix'); ?></a>
                       </li>
                       <li class="howto"><a href="<?php echo esc_url('https://wordpress.org/support/' ); ?>"
                           target="_blank"><?php esc_html_e( 'Learn how to use WordPress','corpix' ); ?></a>
                       </li>
                       <li class="rating"><a href="<?php echo esc_url('https://themeforest.net/downloads' ); ?>"
                          target="_blank"><?php esc_html_e( 'Leave an Item Rating','corpix' ); ?></a></li>
                          <li class="support"><a href="<?php echo esc_url( 'https://pixelcurve.com/support/' ); ?>"
                             target="_blank"><?php esc_html_e( 'Get Help and Support','corpix' ); ?></a></li>
                         </ul>
                     </div>
                 </div>
                 <?php
            ...
            //add_thickbox();
                ...
                // request the token from our bounce url.
                    ...
                    $response = wp_remote_post( $this->oauth_script, array(
                        ...
                        'headers'     => array(),
                        'body'        => array(
                            'oauth_session' => sanitize_text_field($_POST['oauth_session']),
                    ...
                    if ( is_wp_error( $response ) ) {
                        ...
                        $class         = 'error';
                        echo "<div class=\"$class\"><p>" . sprintf( __( 'Something went wrong while trying to retrieve oauth token: %s','corpix' ), $error_message ) . '</p></div>';
                    } else {
                        $token  = @json_decode( wp_remote_retrieve_body( $response ), true );
                        $result = false;
                        if ( is_array( $token ) && ! empty( $token['access_token'] ) ) {
                            $token['oauth_session'] = sanitize_text_field($_POST['oauth_session']);
                ...
                add_settings_section(
                    ...
                    sprintf( __( 'Login for %s updates','corpix' ), $this->envato_username ),
                    ...
                    if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
                        ...
                        <div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
                            ...
                            <p>
                                ...
                                <?php printf( __( 'Configure the Envato Market plugin to get the theme update notification.','corpix' ),  esc_url('#') ); ?> 
                 
                                <?php printf( __( 'If you need any help, please do not hesitate to contact our support desk. <a target="_blank" class="" href="%s">https://pixelcurve.com/support/ </a>','corpix' ),  esc_url('https://pixelcurve.com/support/') ); ?> 
                            </p>      
                        </div>
