...
if (!class_exists('Corpix_Get_Page_Title')) {
    /**
     *  Page Title Area
     *
    ...
    class Corpix_Get_Page_Title
    {
        ...
        public function render_page_title_html()
        {
            ...
            echo '<div class="header__shape_1"></div>';
            echo '<div class="header__shape_2"></div>';
            echo '<div class="header__shape_3"></div>';
            echo '<div class="header__shape_4"></div>';
        ...
        public function get_page_header_attributes()
        {
            ...
            // Parallax
            $parallax_enabled = Corpix::get_mb_option('page_title_parallax', 'mb_page_title_switch', 'on');
            if ($parallax_enabled) {
                wp_enqueue_script('jquery-paroller', get_template_directory_uri() . '/js/jquery.paroller.min.js');
                $parallax_class = ' page_title_parallax';
                $parallax_speed = apply_filters('corpix/page_title/parallax_speed', Corpix::get_mb_option('page_title_parallax_speed', 'mb_page_title_switch', 'on'));
        ...
        public function get_page_header_classes()
        {
            if (
                ...
                if ($this->RWMB_is_active()) {
                    ...
                    $page_title_align = Corpix::get_option($post_type . '_single_title_align');
                    $breadcrumbs_align = Corpix::get_option($post_type . '_single_breadcrumbs_align');
                    $breadcrumbs_block = Corpix::get_option($post_type . '_single_breadcrumbs_block_switch');
                }
            } else {
                $page_title_align = Corpix::get_mb_option('page_title_align', 'mb_page_title_switch', 'on');
                $breadcrumbs_align = Corpix::get_mb_option('page_title_breadcrumbs_align', 'mb_page_title_switch', 'on');
                $breadcrumbs_block = Corpix::get_mb_option('page_title_breadcrumbs_block_switch', 'mb_page_title_switch', 'on');
            }
        ...
        {
            $bg_enabled = Corpix::get_option($this->post_type . '_' . $this->post_query . '__page_title_bg_switch')
                ?? Corpix::get_option('page_title_bg_switch');

            $min_height = Corpix::get_option($this->post_type . '_' . $this->post_query . '__page_title_height')['height'] ?? 'px';
            $min_height = 'px' !== $min_height ? $min_height : Corpix::get_option('page_title_height')['height'];

            $bg_color = Corpix::get_option($this->post_type . '_' . $this->post_query . '__page_title_bg_image')['background-color'] ?? '';
            $bg_color = $bg_color ?: Corpix::get_option('page_title_bg_image')['background-color'];

            $paddings = Corpix::get_option($this->post_type . '_' . $this->post_query . '__page_title_padding') ?: [];
            if (!isset($paddings['padding-top']) || '' === $paddings['padding-top']) {
                $paddings['padding-top'] = Corpix::get_option('page_title_padding')['padding-top'] ?? '';
            }
            if (!isset($paddings['padding-bottom']) || '' === $paddings['padding-bottom']) {
                $paddings['padding-bottom'] = Corpix::get_option('page_title_padding')['padding-bottom'] ?? '';
            }

            $margin_bottom = Corpix::get_option($this->post_type . '_' . $this->post_query . '__page_title_margin')['margin-bottom'] ?? '';
            $margin_bottom = '' !== $margin_bottom ? $margin_bottom : Corpix::get_option('page_title_margin')['margin-bottom'];
        ...
        {
            $bg_enabled = Corpix::get_option('page_title_bg_switch');
            $bg_color = Corpix::get_option('page_title_bg_image')['background-color'];
            $min_height = Corpix::get_option('page_title_height')['height'];
            $paddings = Corpix::get_option('page_title_padding');
            $margin_bottom = Corpix::get_option('page_title_margin')['margin-bottom'] ?? '';
        ...
        {
            if (is_404()) {
                return Corpix::bg_render('404_page__page_title') ?: Corpix::bg_render('page_title');
            }

            if (
                function_exists('is_woocommerce') && is_woocommerce()
                && !empty($bg_shop = Corpix::bg_render($this->post_type . '_' . $this->post_query . '_page_title'))
            ) {
                return !is_product()
                    ? $bg_shop
                    : Corpix::bg_render('shop_single_page_title');
            }

            if (
                $this->post_type
                && !empty($bg_cpt = Corpix::bg_render($this->post_type . '_' . $this->post_query . '__page_title'))
            ) {
                return $bg_cpt;
            }

            return Corpix::bg_render('page_title', 'mb_page_title_switch', 'on');
        }
        ...
        public function get_title_text()
        {
            if (is_home() || is_front_page()) {
                ...
                $title = single_term_title('', false) . esc_html__(' Tag', 'corpix');
            } elseif (is_date()) {
                $title = get_the_time('F Y');
            } elseif (is_author()) {
                $title = esc_html__('Author:', 'corpix') . ' ' . get_the_author();
            } elseif (is_search()) {
                $title = esc_html__('Search', 'corpix');
            } elseif (function_exists('learn_press_is_search') && learn_press_is_search()) {
				$title = esc_html__('Search Courses', 'corpix');
			} elseif (is_404()) {
                $this->page_title_tag = 'h1';
                $title = Corpix::get_option('404_custom_title_switch')
                    ? esc_html(Corpix::get_option('404_page_title_text'))
                    : esc_html__('Error Page', 'corpix');
            } elseif (is_singular('portfolio')) {
                $title = Corpix::get_option('portfolio_title_conditional')
                    ? esc_html__('Portfolio', 'corpix')
                    : esc_html(get_the_title());
                $title = apply_filters('corpix/page_title/portfolio_single', $title);
            } elseif (is_singular('team')) {
                $title = Corpix::get_option('team_title_conditional')
                    ? esc_html__('Team', 'corpix')
                    : esc_html(get_the_title());
                $title = apply_filters('corpix/page_title/team_single', $title);
            } elseif (function_exists('is_product') && is_product()) {
                $title = Corpix::get_option('shop_title_conditional')
                    ? esc_html__('Shop', 'corpix')
                    : esc_html(get_the_title());
                $title = apply_filters('corpix/page_title/shop_single', $title);
            } 

   //          elseif (function_exists('is_learnpress') && is_learnpress()) {
			// 	switch (true) {
			// 		case (learn_press_is_course()):
			// 			$title = Corpix::get_option('learnpress_title_conditional')
   //                          ? esc_html__('Course', 'corpix')
   //                          : esc_html(get_the_title());
   //                      $title = apply_filters('corpix/page_title/course', $title);
			// 			break;
			// 		case (learn_press_is_courses()):
   //                      $title = esc_html__('Archive', 'corpix');
   //                      $title = apply_filters('corpix/page_title/courses', $title);
			// 			break;
			// 	}
			// } 
            ...
            elseif (is_archive()) {
                ...
                elseif (class_exists('SFWD_LMS') && is_post_type_archive('sfwd-courses')) {
                    $title = Corpix::get_option('ld_courses_archive_page_title');
                } 

                elseif (is_tax(['portfolio_tag', 'portfolio-category'])) {
                    $title = esc_html__('Portfolio', 'corpix');
                    $title = apply_filters('corpix/page_title/portfolio_archive', $title);
                } 
                ...
                if (!empty($post)) {
                    ...
                    if ('post' == get_post_type($post)) {
                        ...
                        $title = Corpix::get_option('blog_title_conditional')
                            ? esc_html__('Blog Post', 'corpix')
                            : esc_html(get_the_title($post->ID));
                        $title = apply_filters('corpix/page_title/blog_single', $title);
                    } else {
                        $title = esc_html(get_the_title($post->ID));
                    }
                } else {
                    $title = esc_html__('No Posts', 'corpix');
                }
            }
            ...
            if (
                ...
                $title = apply_filters('corpix/page_title/custom_text', $title);
            }
        ...
        {
            $user_tag = Corpix::get_mb_option('page_title_tag', 'mb_page_title_switch', 'on');
        ...
        {
            $pt_font = Corpix::get_mb_option('page_title_font', 'mb_page_title_switch', 'on');
        ...
        public function is_breadcrumbs_enabled()
        {
            if (
                ...
                return 'on' == $this->mb_page_title_switch
                    ...
                    : Corpix::get_option('blog_single_page_title_breadcrumbs_switch');
            }

            return Corpix::get_mb_option('page_title_breadcrumbs_switch', 'mb_page_title_switch', 'on');
        }

        public function get_breadcrumbs()
        {
            $breadcrumbs_font = Corpix::get_mb_option('page_title_breadcrumbs_font', 'mb_page_title_switch', 'on');
