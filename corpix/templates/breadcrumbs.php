...
if (!class_exists('Corpix_Breadcrumbs')) {
    /**
     * Breadcrumbs area
     *
     *
    ...
    class Corpix_Breadcrumbs
    {
        ...
        {
            $this->text_home     = esc_html__('Home', 'corpix'); // text for the `Home` link
            $this->text_category = esc_html__('Archive by Category "%s"', 'corpix'); // text for a category page
            $this->text_search   = esc_html__('Search Results for "%s"', 'corpix'); // text for a search results page
            $this->text_tag      = esc_html__('Posts Tagged "%s"', 'corpix'); // text for a tag page
            $this->text_author   = esc_html__('Articles Posted by %s', 'corpix'); // text for an author page
            $this->text_404      = esc_html__('Error 404', 'corpix'); // text for the 404 page
            $this->text_page     = esc_html__('Page %s', 'corpix'); // text 'Page N'
            $this->text_cpage    = esc_html__('Comment Page %s', 'corpix'); // text `Comment Page N`
        }
        ...
        public function __construct ()
        {
            ...
            if ( is_home() || is_front_page() ) {
	            ...
	            if( class_exists( 'WooCommerce' ) && ( is_product() || is_product_category() || is_product_tag() || is_cart() || is_checkout())){
		            ...
		            $out .= corpix_woocommerce_breadcrumb($args);
                ...
                // Portfolio Taxonomies Archive
                    ...
                    $archive_link = get_post_type_archive_link(Corpix_Theme_Helper::get_option('portfolio_slug') ?: 'portfolio');
                    $is_category = is_tax('portfolio-category') ? true : false;
                    $tax_name = get_queried_object()->name;

                    if (get_query_var('paged')) {
                        $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'corpix')) . $sep
                            . $before
                            . ($is_category ? sprintf($this->text_category, $tax_name) : sprintf($this->text_tag, $tax_name))
                            . ', ' . sprintf($this->text_page, get_query_var('paged'))
                            . $after;
                    } else {
                        if ($show_current) $out .= $sep . sprintf($link, $archive_link, esc_html__('Portfolio', 'corpix')) . $sep
                            . $before
                            . ($is_category ? sprintf($this->text_category, $tax_name) : sprintf($this->text_tag, $tax_name))
                            . $after;
                    }
                ...
                // Custom Post Type
                    ...
                    function_exists('is_product') && is_product()
                    && function_exists('corpix_woocommerce_breadcrumb')
                ) {
                    $args = [];
                    $args['delimiter'] = $sep;
                    if ($show_home_link) $out .= $sep;
            ...
            echo Corpix_Theme_Helper::render_html($out);
        }
    }

    new Corpix_Breadcrumbs();
}
