    ...
    class Corpix_Theme_Helper
    {
        ...
        public static function get_option(String $name)
        {
            if (class_exists('Redux')) {
                // Customizer
                if (!empty($GLOBALS['corpix_set'])) {
                    $theme_options = $GLOBALS['corpix_set'];
                } else {
                    $theme_options = get_option('corpix_set');
                }
            }

            if (empty($theme_options)) {
                $theme_options = get_option('corpix_default_options');
            }
        ...
        public static function pagination($query = false, $alignment = 'left')
        {
            ...
            return '<ul class="tpc-pagination' . esc_attr($class_alignment) . '">' . $test_comp . '</ul>';
        }
        ...
        {
            return [
               'before' => '<div class="tpc-pagination"><span class="tpc-pagination-text">' . __( 'Pages:', 'corpix' ) . '&nbsp;</span>',
                'after' => '</div>',
        ...
        public static function load_more(
            ...
            $load_more_text = !empty($load_more_text) ? $load_more_text : esc_html__('Load More', 'corpix');
        ...
        public static function get_sidebar_data($name = 'page')
        {
	        ...
	        $name = apply_filters('corpix/theme-helper/get-sidebar-params/cpt', $name);
            ...
            if (
                ...
                $sb_data['container_class'] = $layout != 'none' ? ' tpc-content-sidebar' : '';
        ...
        {
            $class = $sb_data['class'] ?? '';
            $class .= ' tpc_col-' . (12 - ((int) $sb_data['column'] ?? 0));

            echo '<div class="sidebar-container', $class, '" ', ($sb_data['style'] ?? ''), '>';

            if (is_active_sidebar($sb_data['id'])) {
                echo '<aside class="sidebar">';
                    do_action('corpix/theme-helper/render-sidebar/before-widgets');
        ...
        public static function posted_meta_on()
        {
            ...
            printf(
                '<span><time class="entry-date published" datetime="%1$s">%2$s</time></span><span>' . esc_html__('Published in', 'corpix') . ' <a href="%3$s" rel="gallery">%4$s</a></span>',
            ...
            if ($metadata) {
                printf(
                    ...
                    esc_html_x('Full size', 'Used before full size attachment link.', 'corpix'),
            ...
            edit_post_link(
                ...
                sprintf(
                    wp_kses(__('Edit<span class="screen-reader-text"> "%s"</span>', 'corpix'), $kses_allowed_html),
                    get_the_title()
                ),
        ...
        public static function enqueue_css($style)
        {
            ...
            $css = ob_get_clean();
            $css = apply_filters('corpix/enqueue_shortcode_css', $css, $style);
        }
