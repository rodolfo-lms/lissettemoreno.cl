...
if (!class_exists( 'Corpix_Mega_Menu_Waker')) {
    /**
     * Corpix Mega Menu Walker
     *
     *
     * @category Class
     ...
     * @since 1.0.0
     */
    class Corpix_Mega_Menu_Waker extends Walker_Nav_Menu
    {
        /**
         * @since 1.0.0
         */
    ...
    /*-----------------------------------------------------------------------------------*/
    /* Pixelcurve menu fields
    /*-----------------------------------------------------------------------------------*/
    add_action( 'wp_nav_menu_item_custom_fields', 'corpix_add_megamenu_fields', 10, 4 );
    function corpix_add_megamenu_fields( $item_id, $item, $depth, $args )
    {
        ?>
        ...
        <p class="description description-wide field-megamenu-submenu-pos col-6">
            <label for="edit-menu-item-megamenu-submenu-pos-<?php echo esc_attr( $item_id ) ?>">
                <?php esc_html_e( 'Submenu Position', 'corpix' ); ?>
                <select id="edit-menu-item-megamenu-submenu-pos-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-submenu-pos" name="menu-item-tpc-megamenu-submenu-pos[<?php echo esc_attr( $item_id ) ?>]">
                    <option value="default" <?php selected( $item->tpc_megamenu_submenu_pos, 'default' ); ?>><?php esc_html_e( 'Default', 'corpix' ); ?></option>
                    <option value="left" <?php selected( $item->tpc_megamenu_submenu_pos, 'left' ); ?>><?php esc_html_e( 'Left', 'corpix' ); ?></option>
                    <option value="right" <?php selected( $item->tpc_megamenu_submenu_pos, 'right' ); ?>><?php esc_html_e( 'Right', 'corpix' ); ?></option>
                </select>
            </label>
        </p>
        ...
        <div class="clear"></div>
        <div class="mt-20">
            <strong><?php esc_html_e( 'Corpix Mega Menu Settings:', 'corpix' ); ?></strong>
            <em><?php esc_html_e( '(Only for Main Menu)', 'corpix' ); ?></em>
        </div>
        <div class="clear"></div>

        <div class='tpc_accordion_wrapper collapsible close widget_class'>
            <div class='tpc_accordion_heading'>
                <span class='tpc_accordion_title'><?php esc_html_e( 'TPC Mega Menu Settings', 'corpix' ); ?></span>
            ...
            <div class="tpc-mega-menu_wrapper">
                <p class="description description-wide field-megamenu-enable">
                    <label for="edit-menu-item-megamenu-enable-<?php echo esc_attr( $item_id ) ?>">
                        <?php
                        esc_html_e( 'Enable The TPC Mega Menu?', 'corpix' );
                        echo '<select',
                            ...
                            '>',
                            '<option value="">', esc_attr__('Disable', 'corpix'), '</option>',
                            '<option value="links" ', selected( $item->tpc_megamenu_enable, 'links' ), '>', esc_html__('Mega Menu Columns', 'corpix'), '</option>',
                        '</select>';
                        ?>
                    </label>
                </p>
                ...
                <p class="description description-wide field-megamenu-columns">
                    <label for="edit-menu-item-megamenu-columns-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Number of Mega Menu Columns', 'corpix' ); ?>
                ...
                <p class="description description-wide field-megamenu-background-image col-6">
                    <label for="edit-menu-item-megamenu-background-image-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Image', 'corpix' );?>
                        <input type="text" class="corpix_media_url widefat code edit-menu-item-megamenu-background-image" name="menu-item-tpc-megamenu-background-image[<?php echo esc_attr( $item_id ) ?>]" id="edit-menu-item-megamenu-background-image-<?php echo esc_attr( $item_id ) ?>" value="<?php echo esc_attr($item->tpc_megamenu_background_image); ?>">
                    </label>
                    <a href="#" class="button corpix_media_upload"><?php esc_html_e('Upload', 'corpix'); ?></a>
                </p>
                ...
                <p class="description description-wide field-megamenu-background-repeat col-6">
                    <label for="edit-menu-item-megamenu-background-repeat-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Repeat', 'corpix' ); ?>
                        <select id="edit-menu-item-megamenu-background-repeat-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-repeat" name="menu-item-tpc-megamenu-background-repeat[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="no-repeat" <?php selected( $item->tpc_megamenu_background_repeat, 'no-repeat' ); ?>><?php esc_html_e( 'No Repeat', 'corpix' ); ?></option>
                            <option value="repeat" <?php selected( $item->tpc_megamenu_background_repeat, 'repeat' ); ?>><?php esc_html_e( 'Repeat', 'corpix' ); ?></option>
                            <option value="repeat-x" <?php selected( $item->tpc_megamenu_background_repeat, 'repeat-x' ); ?>><?php esc_html_e( 'Repeat X', 'corpix' ); ?></option>
                            <option value="repeat-y" <?php selected( $item->tpc_megamenu_background_repeat, 'repeat-y' ); ?>><?php esc_html_e( 'Repeat Y', 'corpix' ); ?></option>
                        </select>
                    </label>
                </p>
                <div class="clear"></div>
                <p class="description description-wide field-megamenu-background-pos-x col-6">
                    <label for="edit-menu-item-megamenu-background-pos-x-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Position X', 'corpix' ); ?>
                        <select id="edit-menu-item-megamenu-background-pos-x-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-pos-x" name="menu-item-tpc-megamenu-background-pos-x[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="right" <?php selected( $item->tpc_megamenu_background_pos_x, 'right' ); ?>><?php esc_html_e( 'Right', 'corpix' ); ?></option>
                            <option value="center" <?php selected( $item->tpc_megamenu_background_pos_x, 'center' ); ?>><?php esc_html_e( 'Center', 'corpix' ); ?></option>
                            <option value="left" <?php selected( $item->tpc_megamenu_background_pos_x, 'left' ); ?>><?php esc_html_e( 'Left', 'corpix' ); ?></option>
                        </select>
                    </label>
                </p>
                ...
                <p class="description description-wide field-megamenu-background-pos-y col-6">
                    <label for="edit-menu-item-megamenu-background-pos-y-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Background Position Y', 'corpix' ); ?>
                        <select id="edit-menu-item-megamenu-background-pos-y-<?php echo esc_attr( $item_id ) ?>" class="widefat code edit-menu-item-megamenu-background-pos-y" name="menu-item-tpc-megamenu-background-pos-y[<?php echo esc_attr( $item_id ) ?>]">
                            <option value="top" <?php selected( $item->tpc_megamenu_background_pos_y, 'top' ); ?>><?php esc_html_e( 'Top', 'corpix' ); ?></option>
                            <option value="center" <?php selected( $item->tpc_megamenu_background_pos_y, 'center' ); ?>><?php esc_html_e( 'Center', 'corpix' ); ?></option>
                            <option value="bottom" <?php selected( $item->tpc_megamenu_background_pos_y, 'bottom' ); ?>><?php esc_html_e( 'Bottom', 'corpix' ); ?></option>
                        </select>
                    </label>
                </p>
                <div class="clear"></div>
                <p class="description description-wide field-megamenu-min-height col-6">
                    <label for="edit-menu-item-megamenu-min-height-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Min Height', 'corpix' );
                        ?>
                ...
                <p class="description description-wide field-megamenu-width col-6">
                    <label for="edit-menu-item-megamenu-width-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Max Width', 'corpix' );
                        ?>
                 ...
                 <p class="description description-wide field-megamenu-padding-left col-6">
                    <label for="edit-menu-item-megamenu-padding-left-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Padding Left', 'corpix' );
                        ?>
                ...
                <p class="description description-wide field-megamenu-padding-right col-6">
                    <label for="edit-menu-item-megamenu-padding-right-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Padding Right', 'corpix' );
                        ?>
                ...
                <p class="description description-wide field-megamenu-hide-headings">
                    <label for="edit-menu-item-megamenu-hide-headings-<?php echo esc_attr( $item_id ) ?>">
                        <?php esc_html_e( 'Hide Mega Menu Headings?', 'corpix' );?>
    ...
    add_action('wp_update_nav_menu_item', 'corpix_custom_nav_update', 10, 3);
    function corpix_custom_nav_update( $menu_id, $menu_item_db_id, $menu_item_data = [] )
    {
        $fields = corpix_mega_menu_fields();
    ...
    if (!function_exists('corpix_mega_menu_fields')) {

        function corpix_mega_menu_fields()
        {
            return [
    ...
    add_filter( 'wp_edit_nav_menu_walker', 'corpix_custom_nav_edit_walker', 10, 2 );
    function corpix_custom_nav_edit_walker($walker,$menu_id) {
        return 'Corpix_Mega_Menu_Edit_Walker';
    }
     ...
     * @see Walker_Nav_Menu
     */
    class Corpix_Mega_Menu_Edit_Walker extends Walker_Nav_Menu
    {
        /**
        ...
        public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 )
        {
            ...
            if ( ! empty( $item->_invalid ) ) {
                ...
                $title = sprintf( esc_html__( '%s (Invalid)', 'corpix' ), $item->title );
            } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
                $classes[] = 'pending';
                /* translators: %s: title of menu item in draft status */
                $title = sprintf( esc_html__('%s (Pending)', 'corpix'), $item->title );
            }
                ...
                <div class="menu-item-bar">
                    <div class="menu-item-handle">
                        <span class="item-title">
                            <span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo !empty($submenu_text) ? $submenu_text : ''; ?>><?php esc_html_e( 'sub item', 'corpix' ); ?></span></span>
                        <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up" aria-label="<?php esc_attr_e( 'Move up', 'corpix' ) ?>">&#8593;</a>
                                |
                                <a href="<?php
                                    echo esc_url(wp_nonce_url(
                                        add_query_arg(
                                ...
                                ?>" class="item-move-down" aria-label="<?php esc_attr_e( 'Move down', 'corpix'  ) ?>">&#8595;</a>
                            </span>
                            ...
                            ?>" aria-label="<?php esc_attr_e( 'Edit menu item', 'corpix'  ); ?>"><span class="screen-reader-text"><?php esc_html_e( 'Edit', 'corpix' ); ?></span></a>
                        </span>
                    </div>
                </div>
                ...
                <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                    ...
                    <p class="description description-wide">
                        <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Navigation Label', 'corpix' ); ?><br />
                            <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                        </label>
                    </p>
                    <p class="field-title-attribute field-attr-title description description-wide">
                        <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Title Attribute', 'corpix' ); ?><br />
                    ...
                    <p class="field-link-target description">
                        <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                            ...
                            <?php esc_html_e( 'Open link in a new tab', 'corpix' ); ?>
                        </label>
                    </p>
                    <p class="field-css-classes description description-thin">
                        <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'CSS Classes (optional)', 'corpix' ); ?><br />
                            <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                        </label>
                    </p>
                    <p class="field-xfn description description-thin">
                        <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Link Relationship (XFN)', 'corpix' ); ?><br />
                            <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                        </label>
                    </p>
                    <p class="field-description description description-wide">
                        <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                            <?php esc_html_e( 'Description', 'corpix' ); ?><br />
                            <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                            <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'corpix'); ?></span>
                        </label>
                    </p>
                    <?php
                        /*-----------------------------------------------------------------------------------*/
                        /* Pixelcurve Mega Menu
                        /*-----------------------------------------------------------------------------------*/
                        do_action( 'wp_nav_menu_item_custom_fields', $item_id, $item, $depth, $args );
                    ?>

                    <fieldset class="field-move hide-if-no-js description description-wide">
                        <span class="field-move-visual-label" aria-hidden="true"><?php esc_html_e( 'Move', 'corpix' ); ?></span>
                        <button type="button" class="button-link menus-move menus-move-up" data-dir="up"><?php esc_html_e( 'Up one', 'corpix' ); ?></button>
                        <button type="button" class="button-link menus-move menus-move-down" data-dir="down"><?php esc_html_e( 'Down one', 'corpix' ); ?></button>
                        <button type="button" class="button-link menus-move menus-move-left" data-dir="left"></button>
                        <button type="button" class="button-link menus-move menus-move-right" data-dir="right"></button>
                        <button type="button" class="button-link menus-move menus-move-top" data-dir="top"><?php esc_html_e( 'To the top', 'corpix' ); ?></button>
                    </fieldset>
                    ...
                    <div class="menu-item-actions description-wide submitbox">
                        <?php if ( 'custom' != $item->type && $original_title !== false ) : ?>
                            <p class="link-to-original">
                                ...
                                printf( wp_kses( __('Original: %s', 'corpix'), $allowed_html ), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                            </p>
                        <?php endif; ?>
                        ...
                        )); ?>"><?php esc_html_e( 'Remove', 'corpix' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( [ 'edit-menu-item' => $item_id, 'cancel' => time() ], admin_url( 'nav-menus.php' ) ) );
                            ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'corpix'); ?></a>
                    </div>
